#!/usr/bin/env bash
# Fail closed if Raven's active iOS product surface drifts back toward a
# public social network. Legacy model/storage symbols may remain only as inert
# migration inputs until a separately reviewed destructive migration removes
# them. Active parsers, notification formatters, report UI, and legal copy are
# scanned because each can independently reintroduce a social product surface.
set -euo pipefail
...
def forbid(relative: str, needle: str, label: str) -> None:
    if needle in read(relative):
        failures.append(f"{label}: forbidden active marker in {relative}: {needle}")
...
# Root UX: conversation, contacts, network status and settings are allowed;
# public feed/discovery/composer roots are not.
...
# cases may remain for source compatibility, but the parser cannot admit them.
...
# Push admission must be strict: unknown is never silently treated as message,
# and public-social notification types must be dropped before side effects.
...
# Notification sources and realtime toasts have explicit allowlists.
require(
...
# No central behavioral analytics or post-consumption tracking in the active
# binary. Haptics remain local and do not create an engagement dataset.
...
# Settings/help are user-visible release surfaces, so legacy social controls
# and claims are forbidden even when old localization keys remain for a later
# migration cleanup.
settings_files = (
...
# TermsOfServiceView renders all 19 localized sections dynamically. These keys
# are active release UI, not inert localization migration data.
...
# The old shortcut identifier can remain to reject installed shortcuts, but it
# must not navigate into a composer.
