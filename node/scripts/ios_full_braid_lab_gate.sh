#!/usr/bin/env bash
#
# Debug/lab-only Swift XCTest gate for raven-fb-ffi (Full Braid).
...
SCHEME="RAVEN"
CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-$NODE/target/full-braid-ios-lab}"
# Relative paths are resolved against $NODE so inherited env cannot point at the wrong tree.
if [[ "$CARGO_TARGET_DIR" != /* ]]; then
  CARGO_TARGET_DIR="$NODE/$CARGO_TARGET_DIR"
fi
INCLUDE_DIR="$NODE/crates/raven-fb-ffi/include"
XCF_DIR="$CARGO_TARGET_DIR/xcframework"
XCF_NAME="RavenFullBraid.xcframework"
...
'
    ...
    echo "FULL_BRAID_IOS_SIMULATOR_UNAVAILABLE: install an iOS Simulator runtime and create an iPhone device, or pass DEST/DEVICE_ID" >&2
    return 2
  fi
  if [[ -z "$simulator_id" ]]; then
    echo "FULL_BRAID_IOS_SIMULATOR_UNAVAILABLE: simulator discovery returned an empty device id" >&2
    return 2
  fi
...
echo "Full Braid lab destination: $DESTINATION" >&2
...
DEVICE_LIB="$CARGO_TARGET_DIR/$DEVICE_TARGET/debug/libraven_fb_ffi.a"
SIM_ARM_LIB="$CARGO_TARGET_DIR/$SIM_ARM_TARGET/debug/libraven_fb_ffi.a"
SIM_X86_LIB="$CARGO_TARGET_DIR/$SIM_X86_TARGET/debug/libraven_fb_ffi.a"
...
rm -rf "$XCF_DIR"
mkdir -p "$XCF_DIR"
SIM_UNIVERSAL="$XCF_DIR/libraven_fb_ffi-sim-universal.a"
...
FFI_LIBRARY="$XCF_DIR/libraven_fb_ffi-sim-${HOST_SIM_ARCH}.a"
...
(
  ...
  xcodebuild test \
    ...
    'SWIFT_ACTIVE_COMPILATION_CONDITIONS=DEBUG RAVEN_FULL_BRAID_FFI' \
    'OTHER_LDFLAGS=$(inherited) $(RAVEN_FB_LIB_$(PLATFORM_NAME))' \
    "RAVEN_FB_LIB_iphonesimulator=$FFI_LIBRARY" \
...
echo "ios_full_braid_lab_gate: PASS (XCFramework + XCTest)" >&2
