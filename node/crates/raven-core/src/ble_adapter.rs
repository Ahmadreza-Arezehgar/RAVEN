...
//!   only; it does not open a radio. iOS GATT is unverified/absent on
//!   `main` (no Swift / `ios-native`). SoT until B8 (not merged):
//!   `feature/raven-serverless-v1` (`BLEMeshEngine.swift`,
//!   `RavenBleRvn1Carrier.swift`, `RavenBleRvn1CarrierTests.swift`).
//!   `FeatureFlag.ravenEnvelopeV1` default OFF (Phase G / experimental).
//!   Do not treat as present on `main`.
