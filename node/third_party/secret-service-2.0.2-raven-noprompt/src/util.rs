...
pub(crate) fn format_secret(
    ...
    if session.is_encrypted() {
        let mut rng = OsRng {};
        let mut aes_iv = Zeroizing::new([0; 16]);
        rng.fill(aes_iv.as_mut());

        let encrypted_secret = encrypt(secret, session.aes_key()?, aes_iv.as_ref())?;
...
pub(crate) fn exec_prompt(
    ...
    if cfg!(feature = "raven-r0-test-instrumentation") {
