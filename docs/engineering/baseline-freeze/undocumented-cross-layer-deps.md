...
| D20 | BLE service UUIDs only in MESH_PROTOCOL | `docs/MESH_PROTOCOL.md` §A `12345678-1234-1234-1234-123456789ABC` | **R2** — Rust `ble_adapter` exists; **UNKNOWN** whether it uses the same UUIDs without reading the full adapter (not duplicated in `RAVEN_BLE_FRAMING_V1.md`) | #10 + #12 | **document** (copy UUID into BLE V1 or explicitly “legacy only”) |
...
| D27 | Workspace `[patch.crates-io] libsqlite3-sys` → raven fork | `node/Cargo.toml`; `third_party/libsqlite3-sys-raven` | **R2–R3** if a crate outside `node/` expected stock rusqlite | #8 + #5 | **document** (already in Cargo comments); keep **CODEOWNERS** on third_party if missing — **UNKNOWN** (live CODEOWNERS has no `third_party/` line) |
