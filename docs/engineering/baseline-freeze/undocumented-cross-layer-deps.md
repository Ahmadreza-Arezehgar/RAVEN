...
| D10 | `ash` + `raven-node` + `raven-swarm` → same `identity_store` if `data_dir` shared | `IDENTITY_SEED_STORAGE.md`; swarm `load_or_create_identity`; ash/node same APIs | **R3** — swarm derivation of libp2p key from seed (`main.rs` `libp2p_keypair_from_raven`) means **read access to the Raven seed** by the swarm process | Identity Lead (freeze-org #6) + **Role #6 P2P** (swarm) + #5 | **ADR** (seed vs transport-key isolation) + **document** |
| D11 | `raven-swarm` → domain-separated PeerId from Raven seed | `raven-swarm/src/main.rs` `SHA-256(b"raven/libp2p-peer-key/v1" \|\| seed)` | **R2** — documented in comments, not in ADR 0003 beyond “peer id separate.” Compromise of swarm process = seed in memory | #5 + **Role #6 P2P** | **document** in IDENTITY + **ADR** if seed must never enter swarm |
...
| D24 | Kad / mailbox protocol IDs only in Rust | `/raven/kad/1.0.0`, `/raven/offline-mailbox/1.0.0` in swarm sources; mailbox also in `RAVEN_MAILBOX_TRANSPORT_V1.md` | **R2** if RDAP or clients hardcode strings (mesh uses the **binary**, not the protocol id) | **Role #6 P2P** + #2 | **none** for RDAP today; **document** if a second implementation appears |
